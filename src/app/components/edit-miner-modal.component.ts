...
@Component({
      ...
      <div class="modal-card" (click)="$event.stopPropagation()">
        ...
        <form [formGroup]="editForm" (ngSubmit)="onSubmit()">
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn-primary" [disabled]="editForm.invalid">Änderungen speichern</button>
          </div>
        </form>
      </div>
    </div>
  ...
  styles: [`
    ...
    .modal-card {
      background-color: var(--bg-surface); width: 100%; max-width: 480px;
...
export class EditMinerModalComponent implements OnInit {
  ...
  ngOnInit() {
    // Prüfen, ob das Modell ein Standardmodell ist
